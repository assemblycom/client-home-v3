  ...
  const getOptionsForCondition = useCallback(
    (currentCompareValue: string) => {
      ...
      const usedByOtherConditions = new Set(
        conditions.filter((c) => c.compareValue !== currentCompareValue).map((c) => c.compareValue),
      )
      return options
        .filter((opt) => !usedByOtherSegments.has(opt.key) && !usedByOtherConditions.has(opt.key))
              ...
              <div className="flex items-center gap-2">
                {isMultiSelect ? (
                  <Select
                    ...
                    options={getOptionsForCondition(condition.compareValue)}
                    placeholder="Select value"
                    className="flex-1"
                    disabled={getOptionsForCondition(condition.compareValue).length === 0 && !condition.compareValue}
                  />
                ) : (
                  <input
                    type="text"
